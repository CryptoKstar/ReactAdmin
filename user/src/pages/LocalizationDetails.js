...
export default function LocalizationDetails() {
    ...
    const handleSubmit = (e) => {
        ...
        dataProvider.update('company_site_payment_methods_localize', { id: params.id, data: { Languange: ValueObjectLocalize.localize ? ValueObjectLocalize.localize : "en-US", Data: local_data } })
            .then(res => {
                setAlertMessage(t("Selected Payment Methods is updated correctly"));
    ...
    const deleteMethod = () => {
        dataProvider.delete('company_site_payment_methods_localize', { id: params.id })
            .then(res => {
                setAlertMessage(t("Selected Payment Methods is deleted correctly"));
                setAlertType("success");
                setAlertOpen(true);
                history.push('/paymentmethods')
            })
    ...
    const load = () => {
        const Id = params.id;
        console.log(Id);
        dataProvider.getList("company_site_payment_methods_localize", { pagination: { page: 1, perPage: 10 }, sort: { field: 'id', order: 'ASC' }, filter: {} }).then(res => {
            console.log(res)
            const res_data = [];
            const data = res.data;
            ...
            setLocalize(res_data[0].localize);
            // eslint-disable-next-line
            setValueObjectLocalize({ ...ValueObjectLocalize, ["localize"]: res_data[0].localize })
        })
    }
    useEffect(() => {
        load()
